...
function deleteBlog($hn, $un, $pw, $db){
    ...
    $sql = " delete from demoblogs where id = $blogid";
    $conn->query($sql);        
...
function validateUserOwnershipOfBlog($hn, $un, $pw, $db){
    ...
    $sql = "select demoblogs.body from users, demoblogs where users.id = demoblogs.userID and users.id=$id and users.authtoken = '$auth' and demoblogs.id=$blogid";
