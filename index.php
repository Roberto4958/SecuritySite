...
if($_POST){
    ...
    //check if log out was clicked
    if(isset($_POST['logout'])){
        endSession();
        header('Location: logIn');
    }
    
    //check if blog was submitted
...
function addBlog($userId, $username, $title, $content, $hn, $un, $pw, $db){
    ...
    $sql = "insert into blogs(userID, username, title,body, date) values(?, ?, ?, ?, ? )";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param('issss',$userId, $username, $title, $content, $time);
    $stmt->execute();
    
    if($stmt->affected_rows) $success = false;
    $stmt->close();
    $conn->close();
    return $success;
}
...
<div class="container">
    <div class="row">
        <form method= "post">
            Do you have any insite in cyber attacks?<br><br>
